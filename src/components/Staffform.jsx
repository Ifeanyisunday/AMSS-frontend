import { useStaffMutation } from "../apis/Functions"
import { useState } from "react";

const Staffform = () => {

//   const [isUpdate, setIsUpdate] = useState(false)

  const [addStaff] = useStaffMutation()
//   const [updateBusiness] = useUpdateBusinessMutation()

    const [staffInput, setStaffInput] = useState({
        username: "",
        first_name: "",
        last_name: "",
        email: "",
        password: "",
        role: "",
        phone_number: "",
        address: "",
        age: "",
    });

  const handleInputChange = (e) => {
    const { name, value } = e.target;
    setStaffInput({ ...staffInput, [name]: value });
  };



//   const handleUpdate = async(e) => {
//     e.preventDefault();
//     const id = localStorage.getItem('id')
//     try {
//       const response = await updateBusiness({id, businessInput}).unwrap()
//       console.log(response)
//       setBusinessInput({
//         business_name: "",
//         business_address: "",
//         business_type: "",
//         bank_account_details: "",
//         preferred_currency: "",
//         industry: "",
//         annual_revenue: "",
//         business_email: "",
//       })

//       setIsUpdate(false)
//       // navigate("/loginPage")   
//     } catch (err) {
//       if (err?.data?.error) {
//         alert(`Error: ${err.data.error}`);
//       } else {
//         alert("An unexpected error occurred.");
//         console.error(err);
//       }
//     }
//   }

  const handleSubmit = async(e) => {
    e.preventDefault();

    try {
      const response = await addStaff(staffInput).unwrap()
      localStorage.setItem("id", response.id);
      console.log(response)
      setStaffInput({
        username: "",
        first_name: "",
        last_name: "",
        email: "",
        password: "",
        role: "",
        phone_number: "",
        address: "",
        age: "",
      })
      // navigate("/loginPage")   
    } catch (err) {
      if (err?.data?.error) {
        alert(`Error: ${err.data.error}`);
      } else {
        alert("An unexpected error occurred.");
        console.error(err);
      }
    }
  }
  

  return (
    <div className="p-6 bg-gray-100 min-h-screen flex items-center justify-center">
      <div className="max-w-2xl w-full bg-white rounded-lg shadow-md p-8">
        <h2 className="text-3xl font-bold text-center mb-6">Create Staff</h2>

        {/* {submitted && <p className="text-green-600 text-center mb-4">Business created successfully!</p>} */}

        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Business Name */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Staff</label>
            <input
              type="text"
              name="username"
              value={staffInput.username}
              onChange={handleInputChange}
              placeholder="Enter staff name"
              className={`w-full p-2 border rounded`}
            />
            {/* {errors.business_name && <p className="text-red-500 text-sm mt-1">{errors.business_name}</p>} */}
          </div>

          {/* Business Address */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">first name</label>
            <input
              type="text"
              name="first_name"
              value={staffInput.first_name}
              onChange={handleInputChange}
              placeholder="Enter first name"
              className={`w-full p-2 border rounded`}
            />
            {/* {errors.business_address && <p className="text-red-500 text-sm mt-1">{errors.business_address}</p>} */}
          </div>

          {/* Business Type */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">last name</label>
            <input
              type="text"
              name="last_name"
              value={staffInput.last_name}
              onChange={handleInputChange}
              placeholder="Enter last name"
              className={`w-full p-2 border rounded`}
            />
            {/* {errors.business_type && <p className="text-red-500 text-sm mt-1">{errors.business_type}</p>} */}
          </div>

          {/* Bank Account Details */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">email(Optional)</label>
            <input
              type="email"
              name="email"
              value={staffInput.email}
              onChange={handleInputChange}
              placeholder="Enter email"
              className="w-full p-2 border rounded border-gray-300"
            />
          </div>

          {/* Preferred Currency */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">password</label>
            <input
              type="text"
              name="password"
              value={staffInput.password}
              onChange={handleInputChange}
              placeholder="Enter password"
              className="w-full p-2 border rounded border-gray-300"
            />
          </div>

          {/* Industry */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">role</label>
            <input
              type="text"
              name="role"
              value={staffInput.role}
              onChange={handleInputChange}
              placeholder="Enter role"
              className="w-full p-2 border rounded border-gray-300"
            />
          </div>

          {/* Annual Revenue */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">phone number</label>
            <input
              type="text"
              name="phone_number"
              value={staffInput.phone_number}
              onChange={handleInputChange}
              placeholder="Enter phone number"
              className={`w-full p-2 border rounded`}
            />
            {/* {errors.annual_revenue && <p className="text-red-500 text-sm mt-1">{errors.annual_revenue}</p>} */}
          </div>

          {/* Business Email */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">address</label>
            <input
              type="text"
              name="address"
              value={staffInput.address}
              onChange={handleInputChange}
              placeholder="Enter address"
              className={`w-full p-2 border rounded`}
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">age</label>
            <input
              type="text"
              name="age"
              value={staffInput.age}
              onChange={handleInputChange}
              placeholder="Enter age"
              className={`w-full p-2 border rounded`}
            />
          </div>

          {/* Submit Button */}
          <button
            type="submit"
            className="w-full bg-blue-400 text-white p-2 rounded hover:bg-blue-700"
          >
            {"Create"}
          </button>
        </form>
        {/* <button
            className="w-full bg-gray-400 text-white p-2 rounded hover:bg-gray-700 mt-4" onClick={() => setIsUpdate(true)}>
            Update Business
          </button> */}
      </div>
    </div>
  )
}

export default Staffform